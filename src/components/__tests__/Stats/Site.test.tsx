...
describe('Site', () => {
  ...
  it('renders the site stats table', async () => {
    const Component = await Site({ lang: 'en' });
    render(Component);

    expect(screen.getByRole('table')).toBeInTheDocument();
  });

  it('displays static labels based on lang', async () => {
    const ComponentEn = await Site({ lang: 'en' });
    const { unmount } = render(ComponentEn);
    ...
    const ComponentFr = await Site({ lang: 'fr' });
    render(ComponentFr);
  ...
  it('has links for GitHub-sourced stats', async () => {
    ...
    const links = document.querySelectorAll(
      'a[href="https://github.com/fernandohoussou/personal-site/stargazers"]',
    );
    expect(links.length).toBeGreaterThan(0);
  });

  it('displays all expected stat categories in English', async () => {
    const Component = await Site({ lang: 'en' });
    render(Component);
